...
function loadModule(moduleId) {
    ...
    // Load Shiny Live content
    loadShinyliveContent(module.shinyliveContent);
...
function loadShinyliveContent(shinyliveContent) {
    const container = document.getElementById('shinyliveContent');
    if (container) {
        if (shinyliveContent) {
            container.innerHTML = shinyliveContent;
        } else {
            // Contenido por defecto si no existe shinyliveContent
            container.innerHTML = `
                <div class="text-center py-8">
                    <h4 class="text-lg font-semibold theme-text-primary mb-4">🚀 Explora Shiny Live</h4>
                    <p class="theme-text-secondary mb-6">Experimenta con Shiny for Python directamente en tu navegador sin instalación.</p>
                    <a href="https://shinylive.io/py/examples/" target="_blank" class="inline-block px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg font-semibold hover:shadow-lg transform hover:scale-105 transition-all duration-200">
                        <i class="fas fa-external-link-alt mr-2"></i>Abrir Shiny Live Examples
                    </a>
                    <div class="mt-4 p-4 theme-bg-tertiary rounded-lg">
                        <p class="text-sm theme-text-secondary">
                            💡 Explora ejemplos interactivos y experimenta con Shiny for Python
                        </p>
                    </div>
                </div>
            `;
